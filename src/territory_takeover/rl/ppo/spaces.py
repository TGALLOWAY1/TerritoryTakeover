...
- ``grid_planes``: shape ``(N + 2, H, W)``.
  - Channels ``0 .. N - 1``: one per player's OWNED cells (visited
    territory), with the active player rotated to channel ``0`` so every
    input looks symmetric to the network ("I'm always channel 0").
  - Channel ``N``: EMPTY mask.
  - Channel ``N + 1``: one-hot head position of the active player (all zeros
    if the active player has no head).

- ``scalar_features``: shape ``(3 + N,)``.
  - Index 0: normalized turn number (``turn / (H * W)``, clamped to ``[0, 1]``).
  - Indices 1..N: per-player normalized territory counts
    (``territory / (H * W)``), rotated so the active player is index 1.
  - Index N + 1: board fill ratio (``1 - empty / (H * W)``).
  - Index N + 2: the active player's territory fraction (duplicate of
    index 1, kept so the scalar layout matches the AlphaZero encoder).
...
def encode_observation(
    ...
    (vectorized numpy comparisons), which scales well to 40x40 boards.
    """
    num_players = len(state.players)
    h, w = state.grid.shape
    grid = state.grid

    planes = np.zeros((num_players + 2, h, w), dtype=np.float32)
    order = _rotated_player_order(player_id, num_players)

    for out_idx, pid in enumerate(order):
        planes[out_idx] = (grid == OWNED_CODES[pid]).astype(np.float32)

    planes[num_players] = (grid == EMPTY).astype(np.float32)

    head_plane = planes[num_players + 1]
    ...
    for out_idx, pid in enumerate(order):
        scalars[1 + out_idx] = state.players[pid].territory_count / total_cells
    scalars[1 + num_players] = 1.0 - empty_count / total_cells
    scalars[2 + num_players] = state.players[player_id].territory_count / total_cells
